...
Sempre que aparecer jefersonalmeida, você precisa substituir pelo seu usuário no DockerHub.
...
```sh
$ cd redis
$ docker build -t jefersonalmeida/redis:devops .
$ docker run -d --name redis -p 6379:6379 jefersonalmeida/redis:devops
$ docker ps
$ docker logs redis
```
...
```sh
$ cd ../node
$ docker build -t jefersonalmeida/node:devops .
```
Agora iremos rodar a imagem do node, fazendo a ligação dela com o container do Redis.
```sh
$ docker run -d --name node -p 8080:8080 --link redis jefersonalmeida/node:devops
$ docker ps 
$ docker logs node
```
...
```sh
$ cd ../nginx
$ docker build -t jefersonalmeida/nginx:devops .
```
Criando o container do nginx a partir da imagem e fazendo a ligação com o container do Node
```sh
$ docker run -d --name nginx -p 80:80 --link node jefersonalmeida/nginx:devops
$ docker ps
```
...
Linha 8 = jefersonalmeida/nginx:devops
Linha 18 = image: jefersonalmeida/redis:devops
Linha 37 = image: jefersonalmeida/node:devops
